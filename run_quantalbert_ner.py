...
from albert.modeling_albert import AlbertConfig, AlbertForNer
from albert import tokenization_albert
from albert.file_utils import WEIGHTS_NAME
...
from processors import get_label_tag, make_label_map, PADDREC
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                ...
                output_dir = os.path.join(args.output_dir, 'checkpoint-epoch{0}-step{1}'.format(epoch, global_step))
...
def collate_pred(preds, pred_argmax, label_lists, axis = 0):
    """
    Compare preds and new pred shape, reshape and equal

    """
    label_map = make_label_map(label_lists)
    padding_label_id = label_map[PADDREC.PAD_TAG]
    ...
    if preds_cols > argmax_cols:
        ...
        pred_argmax = np.pad(pred_argmax, ((0, 0), (0, padding_num)), \
            'constant', constant_values = (padding_label_id, padding_label_id))
...
def print_eval_output(output_dir, preds, output_labels, label_lists):
    ...
    for i in range(preds_rows):
        ...
        pred_tag = list(map(lambda x: get_label_tag(x, label_lists), pred_label))
        out_tag = list(map(lambda x: get_label_tag(x, label_lists), out_label))
...
def main():
    ...
    model =AlbertForNer.from_pretrained(args.model_name_or_path,
                                                        ...
                                                        config=config)
    quantmodel = torch.quantization.quantize_dynamic(model, {torch.nn.Linear}, dtype = torch.qint8)
    model = quantmodel
